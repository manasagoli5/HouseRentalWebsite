    ...
    <div style="margin: 3%">
        <div class="row">
            ...
            <?php 
                ...
                $sql = "SELECT * FROM $tourTable WHERE UserID='$userID'";
                if(isset($_GET['tourid'])) {
                    $id = $_GET['tourid'];
                    $sql = "DELETE FROM $tourTable WHERE TourID = $id";
                }
                ...
                while( $row = $response->fetch_assoc() ) {
                    ...
                    echo            "<h5 class='card-title'><span style='color:#c32424 !important'>TourID</span>: $row[TourID]</h5>";
                    echo            "<p class='card-text'><span style='color:#518912 !important'>Date</span>: $row[TourDate]</p>";
                    echo            "<p class='card-text'><span style='color:#518912 !important'>Message</span>: $row[TourMessage]</p>";
                    echo "<p><a href='tourdetails.php?id=$row[PropertyID]' class='btn btn-primary'>Details</a><a href='requesttour.php?tourid=$row[TourID]'><i class='material-icons' style='font-size:40px;color:#c32424;float:right'>delete_forever</i></a></p>";
                    echo        "</div>";
                    echo "</div>";
                    echo "</div>";
                }
            ...
            if($response->num_rows == 0) {
                echo "<p style='margin:18%;font-size:30px;text-align:center;color:#c32424'>No Tours For You!</p>";
            }
            ?>
        </div>
        </div>
